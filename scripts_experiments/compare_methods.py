...
from utils.plot_utils import (create_bar_plot, create_parity_plot,
                              create_strip_plot, create_violin_plot,
                              plot_mean_predictions)
...
def compare_results(exp_dir, opt, path1, path2, method1, method2):
    ...
    for outer in range(1, opt.folds + 1):
        ...
        metrics_gnn = extract_metrics(
            file=outer_m1 + f"/performance_outer_test_fold{outer}.txt"
        )
        metrics_tml = extract_metrics(
            file=outer_m2 + f"/performance_outer_test_fold{outer}.txt"
        )
        ...
        for inner in range(1, opt.folds):
            ...
            m1_dir = os.path.join(
                path1, f"Fold_{outer}_test_set", f"Fold_{real_inner}_val_set"
            )
            ...
            m2_dir = os.path.join(
                path2, f"Fold_{outer}_test_set", f"Fold_{real_inner}_val_set"
            )
    ...
    save_dir = (
        f'{opt.log_dir_results}/{exp_dir}/{method1}_vs_{method2.replace(" ", "_")}'
    )
    ...
    create_bar_plot(
        means=(mae_mean_gnn, mae_mean_tml),
        stds=(mae_gnn_std, mae_tml_std),
        min=minimun,
        max=maximun,
        metric="MAE",
        save_path=save_dir,
    ...
    create_bar_plot(
        means=(rmse_mean_gnn, rmse_mean_tml),
        stds=(rmse_gnn_std, rmse_tml_std),
        min=minimun,
        max=maximun,
        metric="RMSE",
        save_path=save_dir,
    ...
    create_bar_plot(
        means=(r2_mean_gnn, r2_mean_tml),
        stds=(r2_gnn_std, r2_tml_std),
        min=minimun,
        max=maximun,
        metric="R2",
        save_path=save_dir,
    ...
    create_bar_plot(
        means=(accuracy_mean_gnn, accuracy_mean_tml),
        stds=(accuracy_gnn_std, accuracy_tml_std),
        min=minimun,
        max=maximun,
        metric="Accuracy",
        save_path=save_dir,
    ...
    create_bar_plot(
        means=(precision_mean_gnn, precision_mean_tml),
        stds=(precision_gnn_std, precision_tml_std),
        min=minimun,
        max=maximun,
        metric="Precision",
        save_path=save_dir,
    ...
    create_bar_plot(
        means=(recall_mean_gnn, recall_mean_tml),
        stds=(recall_gnn_std, recall_tml_std),
        min=minimun,
        max=maximun,
        metric="Recall",
        save_path=save_dir,
    ...
    create_parity_plot(
        data=results_all, save_path=save_dir, method1=method1, method2=method2
    )
    ...
    print(
        "Accuracy: {:.3f}".format(
            accuracy_score(
                gnn_predictions["real_face"], gnn_predictions["predicted_face"]
            )
        )
    )
    print(
    ...
    print(
        "R2: {:.3f}".format(
            r2_score(gnn_predictions["real_ddG"], gnn_predictions["predicted_ddG"])
        )
    )
    print(
        "MAE: {:.3f}".format(
            mean_absolute_error(
                gnn_predictions["real_ddG"], gnn_predictions["predicted_ddG"]
            )
        )
    )
    print(
    ...
    print(
        "Accuracy: {:.3f}".format(
            accuracy_score(
                tml_predictions["real_face"], tml_predictions["predicted_face"]
            )
        )
    )
    print(
    ...
    print(
        "R2: {:.3f}".format(
            r2_score(tml_predictions["real_ddG"], tml_predictions["predicted_ddG"])
        )
    )
    print(
        "MAE: {:.3f}".format(
            mean_absolute_error(
                tml_predictions["real_ddG"], tml_predictions["predicted_ddG"]
            )
        )
    )
    print(
    ...
    results_all = (
        results_all.groupby(["index", "Method"])
        .agg(
