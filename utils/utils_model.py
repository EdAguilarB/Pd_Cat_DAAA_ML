...
def descriptors_all(data: pd.DataFrame):
    ...
    for index, row in data.iterrows():
        ...
        frags = pd.DataFrame(
            count_fragments(Chem.MolFromSmiles(smiles), "substrate"), index=[index]
        )
    ...
    for index, row in data.iterrows():
        ...
        desc = pd.DataFrame(
            calc_descriptors(Chem.MolFromSmiles(smiles), "ligand"), index=[index]
        )
    ...
    X["HA_solv"] = data["solvent_smiles"].apply(
        lambda m: Descriptors.HallKierAlpha(Chem.MolFromSmiles(m))
    )
